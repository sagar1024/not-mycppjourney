...
// Input
// The first line of the input contains one integer t (1≤t≤36) — the number of test cases. The only line of the test case contains one integer x (1≤x≤9999) — the apartment number of the resident who answered the call. It is guaranteed that x consists of the same digit.
...
int main()
{
    ...
    while(t--)
    {
        ...
        int cycleSum = 1+2+3+4;
        int ans = 0;
        ans = (x[0]-'0'-1) * cycleSum;
        int lenX = x.size();
        if(lenX==1)
        {
            ans += 1;
