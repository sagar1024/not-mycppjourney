...
// Note that it is permissible that some student of the first team has the same skill as a student of the second team. Consider some examples (skills are given):
...
// Input
// The first line of the input contains one integer t (1≤t≤104) — the number of test cases. Then t test cases follow. The first line of the test case contains one integer n (1≤n≤2⋅105) — the number of students. The second line of the test case contains n integers a1,a2,…,an (1≤ai≤n), where ai is the skill of the i-th student. Different students can have the same skills. It is guaranteed that the sum of n over all test cases does not exceed 2⋅105 (∑n≤2⋅105).
...
int main()
{
    ...
    while(t--)
    {
        ...
        vector<int> skills(n);
        set<int> distinctSkills;
        for(int i=0; i<n; i++)
        {
            cin >> skills[i];
            distinctSkills.insert(skills[i]);
        }
        ...
        //Calculating the max same skills
        int count = 1;
        int maxCount = 0;
        for(int i=1; i<n; i++)
        {
        ...
        int result = 0;
        int distinctSkillsSize = distinctSkills.size();

        //Calculating the max distinct skills
        if(maxCount<=distinctSkillsSize-1)
        {
            result = min(maxCount, distinctSkillsSize);
        }
        else
        {
            result = min((maxCount-1), (distinctSkillsSize+1));
        }
        cout << result << endl;
    }
