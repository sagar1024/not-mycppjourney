...
// Input
// The first line contains a single number n (2 ≤ n ≤ 1000) — the number of the tram's stops. Then n lines follow, each contains two integers ai and bi (0 ≤ ai, bi ≤ 1000) — the number of passengers that exits the tram at the i-th stop, and the number of passengers that enter the tram at the i-th stop. The stops are given from the first to the last stop in the order of tram's movement.
...
int tramCapacity(vector<pair<int, int>> &stops, int n)
{
    int maxCapacity = 0;
    int currentCapacity = 0;
    for (int i = 0; i < n; ++i)
    {
        int exitingPassengers = stops[i].first;
        int enteringPassengers = stops[i].second;

        currentCapacity -= exitingPassengers;
        currentCapacity += enteringPassengers;
...
int main()
{
    ...
    cout << tramCapacity(stops,n) << endl;
